...
SetProjectileTrailPosition:
    ...
    LDA.B $12 
    PHA 
    LDA.B $14 : CLC : ADC.W $0911 : STA.B $12 
    PLA 
...
.leftYHighByte:
  ...
  + CLC : ADC.B $14 : SEC : SBC.W #$0004 : STA.L $7ED778,X 
    DEY 
    LDA.W $0000,Y 
    XBA 
    BMI .leftXHighByte 
...
.leftXHighByte:
  ...
  + CLC : ADC.B $12 : SEC : SBC.W #$0004 : STA.L $7ED730,X 
    INY 
    LDA.W $0002,Y 
    XBA 
    BMI .rightYHighByte 
...
.rightYHighByte:
  ...
  + CLC : ADC.B $14 : SEC : SBC.W #$0004 : STA.L $7ED79C,X 
    LDA.W $0001,Y 
    XBA 
    BMI .rightXHighByte 
    AND.W #$00FF 
...
Handle_Death_Animation_Flashing:
    PHP : PHB : PHK : PLB 
    REP #$30 
    LDA.W $0DE6 : CMP.W #$0004 : BPL .timerGreaterThan3 
    ASL A 
    TAY 
...
.lessThan9:
    ...
    LDA.W $0DE4 : CMP.W #$0002 : BMI .lessThan2 
    ASL A 
    TAX 
...
HandleConnectingGrapple:
    LDA.W $0A1F : AND.W #$00FF 
    CMP.W #$001A : BNE .notGrabbedByDraygon 
...
HandleConnectingGrapple_Swinging:
    ...
    LDA.W $0AF6 : SEC : SBC.W $0D08 : STA.B $12 
    LDA.W $0AFA : SEC : SBC.W $0D0C : STA.B $14 
    JSL.L CalculateAngleOf_12_14_Offset 
    XBA 
    STA.W $0CFA 
    STA.W $0CFC 
    STZ.W $0D00 
    LDA.W $0CFE : CMP.W #$0040 : BMI .lessThan40 
...
.loop:
    LDA.W $0CFA : CMP.W GrappleBeamSpecialAngles_angle,X : BEQ .specialAngle 
    TXA 
    SEC : SBC.W #$000A : TAX 
    BPL .loop 
    CLC 
...
.specialAngle:
    ...
    LDA.W GrappleBeamSpecialAngles_XOffset,X : CLC : ADC.W $0D08 : STA.W $0AF6 
    LDA.W GrappleBeamSpecialAngles_YOffset,X : CLC : ADC.W $0D0C : STA.W $0AFA 
    LDA.W GrappleBeamSpecialAngles_grappleFunction,X : STA.W $0D32 
    STZ.W $0A30 
    STZ.W $0CF8 
    LDA.W $0AF6 : SEC : SBC.W $0B10 : BMI .checkNegativeX 
    CMP.W #$000D : BMI + 
...
.checkNegativeX:
  ...
  + LDA.W $0AFA : SEC : SBC.W $0B14 : BMI .checkNegativeY 
...
.increaseLength:
    LDA.W $0CFE : CMP.W #$0040 : BPL .fullyLengthened 
...
.right:
    LDA.W $0CFA : AND.W #$FF00 
    CMP.W #$8000 : BNE .checkLiquidPhysics 
...
.straightDown:
  ...
  + XBA 
    AND.W #$00FF 
    CMP.W #$0001 : BMI .resetAngularVelocity 
    RTS 
...
UpdateGrappleSwingAngularVelocity:
    LDA.W $0D26 : CLC : ADC.W $0D28 : CLC : ADC.W $0D2A : STA.W $0D26 
    LDA.W $0CFA 
    EOR.W $0D26 
    BPL .notRising 
...
.slowGrapple:
    STZ.W $0CF8 
    LDA.W $0CFA : AND.W #$FF00 
    CMP.W #$8000 : BNE .setSwingingAnimationFrame 
    LDA.W $0A96 : CMP.W #$0040 : BPL .straightDown 
...
.merge:
  ...
  + AND.W #$FF00 
    CMP.W #$0100 : BNE .notGrappleKicking 
    LDA.W $0A96 : CMP.W #$0040 : BMI .grappleKicking 
    LDA.W #$0010 
...
SetSamusAnimationFrameAndPositionForConnectingGrapple_Stuck:
    ...
    ASL A 
    TAX 
    LDA.W $0D16 : SEC : SBC.W GrappleBeamFireOffsets_NotRunning_OriginX,X : STA.W $0AF6 
...
UpdateGrappleBeamStartPositionDuringGrappleFire:
    ...
    ASL A 
    TAX 
    LDA.W $0A1C : CMP.W #$0049 : BEQ .moonwalking 
...
.moonwalking:
    LDA.W $0AF6 
    PHA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_OriginX,X : STA.W $0D16 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_FlareX,X : STA.W $0D1A 
    LDA.W $0AFA : SEC : SBC.B $16 : PHA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_OriginY,X : STA.W $0D18 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_FlareY,X : STA.W $0D1C 
    BRA .return 
...
.running:
    LDA.W $0AF6 
    PHA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_OriginX,X : STA.W $0D16 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_FlareX,X : STA.W $0D1A 
    LDA.W $0AFA : SEC : SBC.B $16 : PHA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_OriginY,X : STA.W $0D18 
    PLA 
...
UpdateGrappleBeamTiles_IncrementFlareCounter:
    ...
    LDA.W $0CD0 : CMP.W #$0078 : BPL .return 
    INC A 
    STA.W $0CD0 
...
UpdateGrappleBeamTiles:
    ...
    LDA.W $0D40 : CLC : ADC.W #$0200 : STA.W $0D40 
...
.facingLeft:
    LDA.L FlareSpritemapTable_IndexOffsets_facingLeft : CLC : ADC.W $0CD6 : STA.B $16 

  + LDA.W $0D1A : SEC : SBC.W $0911 : STA.B $14 
...
.manualReturn:
    LDA.W $0D32 : CMP.W #GrappleBeamFunction_Inactive : BEQ .clearLiquidPhysics 
    ...
    CMP.B $12 : BPL .clearLiquidPhysics 
...
GrappleBeamFunction_Fire_GotoCancel:
    LDA.W $0A1C : CMP.W #$00F0 : BEQ .draygon 
...
.fire:
    ...
    LDA.W $0A1C : CMP.W #$0049 : BEQ .moonwalkingNotRunning 
...
.moonwalkingNotRunning:
    ...
    LDA.W GrappleBeamFireOffsets_NotRunning_OriginY,X : SEC : SBC.B $16 : STA.W $0D04 
    LDA.W $0AF6 
    PHA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_OriginX,X : STA.W $0D08 
    STA.W $0D16 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_FlareX,X : STA.W $0D1A 
    LDA.W $0AFA : SEC : SBC.B $16 : PHA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_OriginY,X : STA.W $0D0C 
    STA.W $0D18 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_NotRunning_FlareY,X : STA.W $0D1C 
    BRA .merge 
...
.running:
    ...
    LDA.W GrappleBeamFireOffsets_Running_OriginY,X : SEC : SBC.B $16 : STA.W $0D04 
    LDA.W $0AF6 
    PHA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_OriginX,X : STA.W $0D08 
    STA.W $0D16 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_FlareX,X : STA.W $0D1A 
    LDA.W $0AFA : SEC : SBC.B $16 : PHA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_OriginY,X : STA.W $0D0C 
    STA.W $0D18 
    PLA 
    CLC : ADC.W GrappleBeamFireOffsets_Running_FlareY,X : STA.W $0D1C 
...
.merge:
    ...
    STZ.W $0DC0 
    LDA.W $0A58 : CMP.W #SamusMovementHandler_ReleasedFromGrappleSwing : BNE .return 
...
.firing:
    JSR.W RTS_9BB8D4 
    LDA.W $0CFE : SEC : SBC.W $0D00 : STA.W $0CFE 
...
.cancel:
    ...
    LDA.W $0CFA : CMP.W #$8000 : BNE .propelSamus 
...
GrappleBeamFunction_Dropped:
    LDA.W #$0007 : JSL.L QueueSound 
    LDA.W $0A1C : CMP.W #$00B2 : BEQ .clockwise 
...
.notSwinging:
    LDA.W $0B00 : CMP.W #$0011 : BMI .crouching 
    LDA.W $0A1C 
    ASL #3
    TAX 
    LDA.L PoseDefinitions_directionShotsFired,X : AND.W #$00FF 
...
.decelerating:
    ...
    LDA.W #$0040 : SEC : SBC.B $12 : STA.B $12 
    LDA.W $0CFA 
    XBA 
    AND.W #$00FF 
