...
CrashViewer:
{
    ...
    PEI.b (DP_TempC1) : PEI.b (DP_TempC3)
    JSL cm_transfer_custom_tileset
    PLA : STA.b DP_TempC3 : PLA : STA.b DP_TempC1
    ...
    LDA.b DP_Controller1New : STA !ram_crash_input_prev
...
if !FEATURE_KEEP_UNREFERENCED == 0
    LDA.w #$2800|'U' : STA !ram_tilemap_buffer,X
    INX #2
endif
if !DEBUG
...
+   STA !ram_crash_bytes_to_write
    ...
    %a16()
    LDA !ram_crash_stack+$01 : DEC : STA.b DP_TempC1
    DEC : STA !ram_crash_draw_value
    LDX #$02EC : JSR crash_draw4 ; addr

    LDA.b [DP_TempC1] : STA !ram_crash_draw_value
...
+   INC : STA !ram_crash_cursor
  ...
  .drawMemViewer
    ...
    LDA #$2C80 : STA !ram_tilemap_buffer,X ; arrow pointing left
    LDA #$2C81 : STA !ram_tilemap_buffer+$32,X ; arrow pointing right
    ...
    %a16()
    PEI.b (DP_TempC1) : PEI.b (DP_TempC3)
    LDA !ram_crash_mem_viewer_bank : STA.b DP_TempC3
    LDA !ram_crash_mem_viewer : STA.b DP_TempC1 : STA !ram_crash_temp
    LDA.b [DP_TempC1] : STA !ram_crash_draw_value
    ...
    LDA.b DP_TempC1 : AND #$F0 : STA.b DP_TempC1
  ...
  .drawLowerHalfNearby
    ...
    %a16()
    LDA.b DP_TempC1 : CMP !ram_crash_temp : BNE .incLower
...
+   LDA !ram_crash_loop_counter : INC : STA !ram_crash_loop_counter
  ...
  .drawUpperHalf
    %a16()
    PEI.b (DP_Temp40) : PEI.b (DP_Temp42)
    LDA !ram_crash_mem_viewer_bank : STA.b DP_Temp42
    LDA !ram_crash_mem_viewer : STA.b DP_Temp40 : STA !ram_crash_temp
    LDA.b [DP_Temp40] : STA !ram_crash_draw_value
    LDX #$01E8 : JSR crash_draw4

    LDX #$048A
    %a8()
    LDA #$00 : STA !ram_crash_stack_line_position : STA !ram_crash_loop_counter
    LDA.b DP_Temp40 : AND #$F0 : STA.b DP_Temp40
  ...
  .drawUpperHalfNearby
    ...
    ; highlight selected byte
    %a16()
    LDA.b DP_Temp40 : CMP !ram_crash_temp : BNE .incUpper
    LDA !ram_tilemap_buffer,X : ORA #$1000 : STA !ram_tilemap_buffer,X
    LDA !ram_tilemap_buffer+2,X : ORA #$1000 : STA !ram_tilemap_buffer+2,X
...
+   LDA !ram_crash_loop_counter : INC : STA !ram_crash_loop_counter
  ...
  .doneUpperHalf
    PLA : STA.b DP_Temp42 : PLA : STA.b DP_Temp40
    RTS
...
crash_draw_text:
{
    ...
    PEI.b (DP_TempC5) : PEI.b (DP_TempC7)
    LDA !ram_crash_text_bank : STA.b DP_TempC7
    LDA !ram_crash_text : STA.b DP_TempC5
    %a8()
    LDY #$0000

  .loop
    LDA.b [DP_TempC5],Y : CMP #$FF : BEQ .end   ; terminator
  ...
  .end
    %a16()
    PLA : STA.b DP_TempC7 : PLA : STA.b DP_TempC5
    RTS
}
...
crash_draw4:
{
    ...
    LDA.w CrashHexMenuGFXTable,Y : STA !ram_tilemap_buffer,X
    ; (0X00)
    LDA !ram_crash_draw_value : AND #$0F00 : XBA : ASL : TAY
    LDA.w CrashHexMenuGFXTable,Y : STA !ram_tilemap_buffer+2,X
    ; (00X0)
    LDA !ram_crash_draw_value : AND #$00F0 : LSR #3 : TAY
    LDA.w CrashHexMenuGFXTable,Y : STA !ram_tilemap_buffer+4,X
    ; (000X)
    LDA !ram_crash_draw_value : AND #$000F : ASL : TAY
    LDA.w CrashHexMenuGFXTable,Y : STA !ram_tilemap_buffer+6,X
    RTS
}
...
crash_cycle_palettes:
{
  ...
  .white
    LDA #$44E5 : STA.l Palettes_BG3P2MinimapExplored
    LDA #$7FFF : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .grey
    LDA #$1CE7 : STA.l Palettes_BG3P2MinimapExplored
    LDA #$3DEF : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .green
    LDA #$0000 : STA.l Palettes_BG3P2MinimapExplored
    LDA #$03E0 : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .pink
    LDA #$0000 : STA.l Palettes_BG3P2MinimapExplored
    LDA #$5156 : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .yellow
    LDA #$7C00 : STA.l Palettes_BG3P2MinimapExplored
    LDA #$03FF : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .blue
    LDA #$7FFF : STA.l Palettes_BG3P2MinimapExplored
    LDA #$7A02 : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .red
    LDA #$001F : STA.l Palettes_BG3P2MinimapExplored
    LDA #$7FE0 : STA.l Palettes_BG3P2MinimapExploredFeature
    BRA .transfer

  .orange
    LDA #$7C60 : STA.l Palettes_BG3P2MinimapExplored
    LDA #$01FF : STA.l Palettes_BG3P2MinimapExploredFeature

  .transfer
    LDA.l Palettes_BG3P2MinimapExplored : STA.l Palettes_BG3P3MinimapUnexplored
    LDA.l Palettes_BG3P2MinimapExploredFeature : STA.l Palettes_BG3P3MinimapUnexploredFeature

    LDA #$001F : STA.l Palettes_BG3P7MinimapRoomHighlight ; red highlight
    LDA #$0000 : STA.l Palettes
    STA.l Palettes+$16 : STA.l Palettes+$1E
    STA.l Palettes+$3C
...
{
    %a8()
    LDA.w NMI_Counter : PHA
-   CMP.w NMI_Counter : BEQ -
    PLA : STA.w NMI_Counter
    %a16()
    RTL
}
...
CrashTextInfo8:
    db "smdisassembly.spazer.link", #$FF
