...
GenerateRandomNumber:
    ...
    LDA.W $4216 ; A += ([random number high] * 5 + 1) * 100h
    SEC 
    ADC.B $02,S : STA.B $02,S 
    REP #$20 
    PLA 
    ADC.W #$0011 : STA.W $05E5 ; Random number = [A] + 11h
    RTL 
...
A_Y_16bit_UnsignedMultiplication:
    ...
    CLC : ADC.W $4216 : STA.W $05F2 
    LDY.W $05EC 
    STY.W $4202 
    LDY.W $05E9 
    STY.W $4203 
    NOP ; Result += ad * 100h
    LDA.W $05F2 : CLC : ADC.W $4216 : STA.W $05F2 
    LDY.W $05EA 
    STY.W $4203 
    NOP #2
    LDA.W $05F3 ; Result += bd * 10000h
    CLC : ADC.W $4216 : STA.W $05F3 
    REP #$30 
    PLX 
    RTL 
...
.fadeOut:
    ...
    SEP #$30 
    LDA.B $51 : AND.B #$0F ; If (brightness) = 0: return
    BEQ .return 
    DEC A 
...
Finalise_OAM:
    ...
    STA.B $12 
    LSR A 
    ADC.B $12 : CLC : ADC.W #.spriteY00F0 : STA.B $12 
    LDA.W #$00F0 
    SEP #$30 
    JMP.W ($0012) 
...
.continue:
    ...
    JSR.W CalculateLayer2XPosition 
    BCS .layer2Y 
    CLC : ADC.W $0921 : STA.B $B5 

.layer2Y:
    JSR.W CalculateLayer2YPosition 
    BCS UpdateBGGraphics_WhenScrolling 
    CLC : ADC.W $0923 : STA.B $B7 
...
.updateLayer1:
    TXA 
    CLC : ADC.W $08F7 : STA.W $0990 
    TXA 
    CLC : ADC.W $0907 : STA.W $0994 
    LDA.W $08F9 : STA.W $0992 
    LDA.W $0909 : STA.W $0996 
    JSR.W UpdateLevelDataColumn 
...
.updateLayer2:
    TXA 
    CLC : ADC.W $08FB : STA.W $0990 
    TXA 
    CLC : ADC.W $090B : STA.W $0994 
    LDA.W $08FD : STA.W $0992 
    LDA.W $090D : STA.W $0996 
    JSR.W UpdateBackgroundDataColumn 
...
.layer2HorizontalEnd:
  ...
  + TXA 
    CLC : ADC.W $08F9 : STA.W $0992 
    TXA 
    CLC : ADC.W $0909 : STA.W $0996 
    LDA.W $08F7 : STA.W $0990 
    LDA.W $0907 : STA.W $0994 
...
.finish:
    TXA 
    CLC : ADC.W $08FD : STA.W $0992 
    TXA 
    CLC : ADC.W $090D : STA.W $0996 
    LDA.W $08FB : STA.W $0990 
    LDA.W $090B : STA.W $0994 
...
.unboundedFromAbove:
    ...
    LDA.W $0939 : AND.W #$FF00 
    CLC : ADC.W #$0100 : BRA .returnLayer1Y 
...
UpdateLevelBackgroundDataColumn:
  ...
  + PHP 
    ...
    PHB 
    REP #$30 
    LDA.W $0990 : CLC : ADC.W $4216 : ASL A 
    CLC : ADC.W #$0002 : TXY 
    BEQ + 
    CLC : ADC.W #$9600 ; $7F
  ...
  + CLC : ADC.W $0956,X : STA.W $095E,X 
    CLC : ADC.W #$0040 : STA.W $0960,X 
    STY.W $0937 
    SEP #$20 
    LDA.B #$7E 
    PHA : PLB 
...
UpdateBackgroundLevelDataRow:
  ...
  + CLC : ADC.W $0964,X : STA.W $096C,X 
    CLC : ADC.W #$0044 : STA.W $096E,X 
    STY.W $0937 
    SEP #$20 
    LDA.B #$7E 
    PHA : PLB 
...
DoorTransitionScrollingSetup_Right:
    JSR.W CalculateLayer2XPosition 
    SEC : SBC.W #$0100 : STA.W $0917 
    JSR.W CalculateLayer2YPosition 
    LDA.W $0911 : SEC : SBC.W #$0100 : STA.W $0911 
...
DoorTransitionScrollingSetup_Left:
    JSR.W CalculateLayer2XPosition 
    CLC : ADC.W #$0100 : STA.W $0917 
    JSR.W CalculateLayer2YPosition 
    LDA.W $0911 : CLC : ADC.W #$0100 : STA.W $0911 
    JSR.W UpdateBGScrollOffsets 
    JSR.W Calculate_BGScroll_LayerPositionBlocks 
    JSR.W UpdatePreviousLayerBlocks 
    INC.W $08FF 
...
DoorTransitionScrollingSetup_Down:
    JSR.W CalculateLayer2XPosition 
    JSR.W CalculateLayer2YPosition 
    SEC : SBC.W #$00E0 : STA.W $0919 
    LDA.W $0915 : SEC : SBC.W #$00E0 : STA.W $0915 
    JSR.W UpdateBGScrollOffsets 
    JSR.W Calculate_BGScroll_LayerPositionBlocks 
    JSR.W UpdatePreviousLayerBlocks 
    DEC.W $0901 
...
DoorTransitionScrollingSetup_Up:
    ...
    LDA.W $0915 
    PHA 
    CLC : ADC.W #$001F : STA.W $0915 
    JSR.W CalculateLayer2YPosition 
    CLC : ADC.W #$00E0 : STA.W $0919 
    PLA 
    CLC : ADC.W #$0100 : STA.W $0915 
    JSR.W UpdateBGScrollOffsets 
    LDA.W $0929 : CLC : ADC.W #$0020 : STA.W $0929 
    JSR.W Calculate_BGScroll_LayerPositionBlocks 
    JSR.W UpdatePreviousLayerBlocks 
    INC.W $0901 
    INC.W $0905 
...
UpdateBGScrollOffsets:
    LDA.B $B1 : SEC : SBC.W $0911 : STA.W $091D 
    LDA.B $B3 : SEC : SBC.W $0915 : STA.W $091F 
    LDA.B $B5 : SEC : SBC.W $0911 : STA.W $0921 
    LDA.B $B7 : SEC : SBC.W $0915 : STA.W $0923 
    RTS 
...
DoorTransitionScrolling_Down:
    ...
    LDA.W $0915 
    PHA 
    SEC : SBC.W #$000F : STA.W $0915 
    LDA.W $0919 
    PHA 
    SEC : SBC.W #$000F : STA.W $0919 
    JSR.W Calculate_BGScroll_LayerPositionBlocks 
    JSR.W UpdatePreviousLayerBlocks 
    DEC.W $0901 
    DEC.W $0905 
...
DoorTransitionScrolling_Up:
    ...
    LDA.W $0915 
    PHA 
    SEC : SBC.W #$0010 : STA.W $0915 
    LDA.W $0919 
    PHA 
    SEC : SBC.W #$0010 : STA.W $0919 
    JSR.W Calculate_BGScroll_LayerPositionBlocks 
    JSR.W UpdatePreviousLayerBlocks 
    INC.W $0901 
    INC.W $0905 
...
UNUSED_ConfigureMode7RotationMatrix_80B0C2:
    ...
    INC A 
    STA.B $7C 
    LDA.W $0785 : CLC : ADC.W #$0040 : AND.W #$00FF 
    ASL A 
    TAX 
...
.dictionaryVariant:
  ...
  + STX.B $47 
    ...
    STA.B $4B 
    REP #$20 
    LDA.B $4C : CLC : ADC.B $4A : STA.B $4A 
...
LoadFromLoadStation:
    ...
    STA.B $12 
    ASL A 
    CLC : ADC.B $12 : ADC.W $078B : ASL A 
    CLC : ADC.W LoadStationListPointers,X : TAX 
    ...
    STA.W $0B14 
    LDA.W $0911 : CLC : ADC.W #$0080 : ADC.W $000C,X : STA.W $0AF6 
    STA.W $0B10 
    STZ.B $B1 
    STZ.B $B3 
    SEP #$20 
