...
.zoomOut:
    LDA.W $198F : CMP.W #$7000 : BPL .return 
    INC A 
    STA.W $198F 
    BRA .return 
...
HandleMode7TransformMatrixAndBG1Position:
    PHP 
    REP #$30 
    LDA.W $198D : CLC : ADC.W #$0040 : AND.W #$00FF 
    ASL A 
    TAX 
...
Multiplication_16bitUnsigned_8B85EE:
    ...
    REP #$20 
    LDA.W $4216 : STA.B $2A 
    LDA.B $2E : CLC : ADC.B $30 : STA.B $2E 
    SEP #$20 
    LDA.B $2E : CLC : ADC.B $2D : STA.B $2D 
    LDA.B $2F : ADC.B $2A : STA.B $2A 
    BCC .return 
    INC.B $2B 
...
.loop:
  ...
  + LDA.W $0018 : STA.W $0012 
    DEC.W $0014 
    BEQ .return 
    LDA.W $0016 : CLC : ADC.W #$0040 : STA.W $0016 
    TAX 
    BRA .loop 
...
.loop:
    ...
    TYA 
    CLC : ADC.W $0012 : TAY 
    LDA.W $0016 : CLC : ADC.W #$0080 : STA.W $0016 
    DEC.W $0014 
    BEQ .return 
    BRA .loop 
...
Calculate_SamusPosition_InRotatingElevatorRoom:
    ...
    LDA.B $80 : CLC : ADC.B $1A : STA.W $0AF6 
    LDA.B $7C : STA.B $26 
    LDA.B $22 : STA.B $28 
    JSR.W Multiplication_16bitSigned_8B858F 
    SEP #$20 
...
Calculate_ProjectilePosition_InRotatingElevatorRoom:
    ...
    REP #$20 
    CLC : ADC.B $1A : STA.B $1A 
    LDA.B $80 : CLC : ADC.B $1A : SEC : SBC.W $0911 : STA.B $14 
    LDA.B $7C : STA.B $26 
    LDA.B $22 : STA.B $28 
    JSR.W Multiplication_16bitSigned_8B858F 
    SEP #$20 
...
Calculate_CeresSteamPosition_InRotatingElevatorRoom:
    ...
    REP #$20 
    CLC : ADC.B $1A : STA.B $1A 
    LDA.B $80 : CLC : ADC.B $1A : STA.B $12 
    LDA.B $7C : STA.B $26 
    LDA.B $22 : STA.B $28 
    JSR.W Multiplication_16bitSigned_8B858F 
    SEP #$20 
...
.loop:
    LDA.L $7E2400,X : SEC : SBC.L $7E2A00,X : STA.L $7E2400,X 
    LDA.L $7E2600,X : SEC : SBC.L $7E2C00,X : STA.L $7E2600,X 
    LDA.L $7E2800,X : SEC : SBC.L $7E2E00,X : STA.L $7E2800,X 
    INX #2
    DEY 
    BNE .loop 
    PLP 
...
.loop:
    LDA.L $7E2400,X : CLC : ADC.L $7E2A00,X : AND.W #$1FFF 
    STA.L $7E2400,X 
    LDA.L $7E2600,X : CLC : ADC.L $7E2C00,X : AND.W #$1FFF 
    STA.L $7E2600,X 
    LDA.L $7E2800,X : CLC : ADC.L $7E2E00,X : AND.W #$1FFF 
    STA.L $7E2800,X 
    INX #2
    DEY 
    BNE .loop 
...
MoveUnusedSpritesOffScreen:
    PHP 
    REP #$30 
    LDA.W $0590 : CMP.W #$0200 : BPL .return 
    LSR #2
    PHA 
    AND.W #$0007 
    ASL A 
...
.setXpos:
    ...
    STA.B $12 
    LSR A 
    ADC.B $12 : CLC : ADC.W #.moveAllSprites : STA.B $12 
    LDA.W #$0080 
    SEP #$30 
    JMP.W ($0012) 
...
AdvanceFastScreenFadeIn:
    PHP 
    SEP #$30 
    LDA.B $51 : CLC : ADC.W $0725 : AND.B #$1F 
    CMP.B #$0F : BPL .disableForcedBlank 
    STA.B $51 
    PLP 
    CLC 
    RTS 
...
.loop:
    ...
    LDA.W $1A9D,X : SEC : SBC.W $0915 : STA.B $12 
    BIT.W #$FF00 
    BNE + 
...
.next:
    ...
    BPL .loop 
    PLB 
    LDA.W $1F51 : CMP.W #CinematicFunc_Ending_ZebesDestruction2_CrossFade_FadingIn : BPL .return 
...
.loop:
  ...
  + LDX.W $1A47 
    LDA.W $1A37,X : CMP.W #$0C00 : BEQ .return 
...
Process_CreditsObject_InstList:
    ...
    SEC : SBC.W $1A03 : CMP.W #$0008 : BMI .return 
...
SkipToTitleScreenCheck:
    LDA.W $1F51 : CMP.W #RTS_8B9F28 : BPL .return 
    LDA.B $8F : BIT.W #$9080 
    BEQ .return 
...
CinematicFunction_TitleSequenceScene0_PanningLeft_Lower:
    LDA.W $1991 : CLC : ADC.W $1999 : STA.W $1991 
    LDA.W $1993 : ADC.W $199B : STA.W $1993 
...
CinematicFunction_TitleSequenceScene2_PanningDown:
    LDA.W $1995 : CLC : ADC.W $199D : STA.W $1995 
    LDA.W $1997 : ADC.W $199F : STA.W $1997 
...
CinematicFunction_TitleSequenceScene3_ZoomingOut:
    ...
    BNE .return 
    LDA.W $198F : CMP.W #$0100 : BPL .finish 
    INC A 
    STA.W $198F 
...
PreInstruction_CinematicSpriteObject_MetroidEgg:
    LDA.W $0AF6 : CMP.W #$00A9 : BPL .return 
...
InitFunction_CinematicSpriteObject_MetroidEggParticles:
    ...
    ASL #2
    TAX 
    LDA.W .Xposition,X : CLC : ADC.W #$0010 : STA.W $1A7D,Y 
    LDA.W .Yposition,X : CLC : ADC.W #$003B : STA.W $1A9D,Y 
    LDA.W #$0E00 : STA.W $1ABD,Y 
    RTS 
...
PreInstruction_CinematicSpriteObject_MetroidEggParticle:
    ...
    ASL #2
    TAY 
    LDA.W $1AFD,X : CLC : ADC.W .Yvelocities+2,Y : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W .Yvelocities,Y : STA.W $1A9D,X 
...
.lessThanA8:
    LDA.W $1B7D,X : CLC : ADC.W #$0100 : STA.W $1B7D,X 
    RTS 
...
PreInstruction_CinematicSpriteObject_MetroidEggSlimeDrops:
    ...
    ASL #2
    TAY 
    LDA.W $1AFD,X : CLC : ADC.W .YvelocitiesEven+2,Y : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W .YvelocitiesEven,Y : STA.W $1A9D,X 
...
.nonZero:
    ...
    ASL #2
    TAY 
    LDA.W $1AFD,X : CLC : ADC.W .YvelocitiesOdd+2,Y : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W .YvelocitiesOdd,Y : STA.W $1A9D,X 
    CMP.W #$00A8 : BMI .notHitGround 
...
.crossFading:
    LDA.W $1A49 : BIT.W #$0003 
    BNE .return 
    LDA.W $1997 : CMP.W #$0008 : BPL .return 
    INC A 
    STA.W $1997 
    LDA.W $1A9D,X 
    DEC A 
...
PreInstruction_CinematicBGObject_SamusBlinking:
    LDA.W $1F51 : CMP.W #CinematicFunction_Intro_Page6 : BEQ .notPage6 
...
UNUSED_CinematicBGObject_8BB4DC:
    LDA.W $1B3B : CMP.W #InstList_IntroTextCaret_Blink : BMI .return 
    LDA.W #$0001 : STA.W $19DD,X 
...
.missile:
    LDA.W $0B64,Y : CMP.W #$0054 : BPL .return 
    PHX 
    TYX 
...
.subtract4:
    LDA.W $1997 : SEC : SBC.W #$0004 : STA.W $1997 
...
InitFunction_CinematicSpriteObject_IntroRinka:
    ...
    LDA.W .Yposition,X : SEC : SBC.W #$0008 : STA.W $1A9D,Y 
    LDA.W #$0E00 : STA.W $1ABD,Y 
    RTS 
...
PreInstruction_IntroRinka_Moving_HitsSamus:
    LDA.W $1ADD,X : CLC : ADC.W #$8000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0000 : STA.W $1A7D,X 
    LDA.W $1AFD,X : CLC : ADC.W #$8000 : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W #$0000 : STA.W $1A9D,X 
    LDA.W $0AF6 : SEC : SBC.W #$0005 : STA.B $12 
    LDA.W $1A7D,X : CLC : ADC.W #$0008 : CMP.B $12 : BMI .exploding 
...
.exploding:
    LDA.W $1B3D : CMP.W #PreInstruction_IntroMotherBrain_Exploding : BNE .return 
...
InitFunc_CinematicSpriteObject_IntroMotherBrainExplosion_Big:
    ...
    ASL A 
    TAX 
    LDA.W #$0038 : CLC : ADC.W .Xposition,X : STA.W $1A7D,Y 
    LDA.W #$006F : CLC : ADC.W .Yposition,X : STA.W $1A9D,Y 
...
PreInstruction_CinematicSpriteObject_ConfusedBabyMetroid:
    LDA.W $1B1F,X : CMP.W #InstList_MetroidEggHatching_2 : BMI .return 
...
PreInstruction_ConfusedBabyMetroid_Hatched:
  ...
  + LDA.W $0AFA : SEC : SBC.W #$0020 : CMP.W $1A9D,X : BMI + 
    LDA.W $1A4F : CMP.W #$0220 : BPL .setYVelocity 
    CLC : ADC.W #$0020 : STA.W $1A4F 
    BRA .setYVelocity 


  + LDA.W $1A4F : CMP.W #$FDE1 : BMI .setYVelocity 
...
.timerExpired:
  ...
  + LDA.W $0AF6 : CMP.W $1A7D,X : BMI .checkXposition 
    LDA.W $1A4D : CMP.W #$0280 : BPL + 
    CLC : ADC.W #$0020 : STA.W $1A4D 
    BRA + 


.checkXposition:
    LDA.W $1A4D : CMP.W #$FD81 : BMI + 
...
.setXposition:
    LDA.W $1ADD,X : CLC : ADC.B $14 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.B $12 : STA.W $1A7D,X 
    LDA.W $0AFA : SEC : SBC.W #$0008 : CMP.W $1A9D,X : BMI .checkY 
    LDA.W $1A4F : CMP.W #$0220 : BPL .setYvelocity 
    CLC : ADC.W #$0020 : STA.W $1A4F 
    BRA .setYvelocity 
...
CinematicFunction_FlyToCeres_FlyingIntoCamera:
    LDA.W $198F : CMP.W #$0020 : BMI .zoomLessThan20 
...
PreInstruction_CinematicSpriteObject_CeresStars:
    LDA.W $1F51 : CMP.W #CinematicFunction_FlyToCeres_FlyingIntoCamera : BEQ .flyingIntoCamera 
    RTS 


.flyingIntoCamera:
    LDA.W $1B7D,X : CLC : ADC.W #$0080 : STA.W $1B7D,X 
    XBA 
    PHA 
    AND.W #$FF00 
    STA.B $14 
  ...
  + STA.B $12 
    ...
    LDA.W $1ADD,X : CLC : ADC.B $14 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.B $12 : STA.W $1A7D,X 
    LDA.W $1991 : CLC : ADC.B $14 : STA.W $1991 
    LDA.W $1993 : ADC.B $12 : STA.W $1993 
    LDA.W $1995 : CLC : ADC.B $14 : STA.W $1995 
    LDA.W $1997 : ADC.B $12 : STA.W $1997 
    RTS 
...
PreInst_CinematicSpriteObject_CeresExplosionLargeAsteroids:
    LDA.W $1ADD,X : CLC : ADC.W #$4000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0000 : AND.W #$01FF 
    STA.W $1A7D,X 
    RTS 
...
PreInstruction_CinematicSpriteObject_CeresUnderAttack:
    LDA.W $1ADD,X : CLC : ADC.W #$1000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0000 : AND.W #$01FF 
    STA.W $1A7D,X 
    RTS 
...
PreInstruction_CinematicSpriteObject_CeresSmallAsteroid:
    LDA.W $1ADD,X : CLC : ADC.W #$0800 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0000 : AND.W #$01FF 
    STA.W $1A7D,X 
    RTS 
...
PreInstruction_CinematicSpriteObject_CeresPurpleSpaceVortex:
    LDA.W $1ADD,X : SEC : SBC.W #$2000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : SBC.W #$0000 : STA.W $1A7D,X 
...
CinematicFunction_FlyToCeres_FlyingIntoCeres:
    SEP #$20 
    LDA.B $74 : SEC : SBC.B #$01 : STA.B $74 
    CMP.B #$20 : BPL .color1 
    LDA.B #$20 : STA.B $74 

.color1:
    LDA.B $75 : SEC : SBC.B #$01 : STA.B $75 
    CMP.B #$40 : BPL .color2 
    LDA.B #$40 : STA.B $75 

.color2:
    LDA.B $76 : SEC : SBC.B #$01 : STA.B $76 
...
.zoomLessThanC00:
    ...
    LDA.W $198F : CMP.W #$0C00 : BPL .zoomLessThan2000 
    CLC : ADC.W #$0010 : STA.W $198F 
    RTS 


.zoomLessThan2000:
    LDA.W $198F : CMP.W #$2000 : BPL + 
...
.loopVRAM:
    ...
    LDA.W #$0000 : JSL.L QueueMusicDataOrTrack_8FrameDelay 
    LDA.W #$FF2D : JSL.L QueueMusicDataOrTrack_8FrameDelay 
    LDA.W $0998 : CMP.W #$0025 : BNE .notState25 
    LDA.W #$0008 
    LDY.W #$000E 
    JSL.L QueueMusicDataOrTrack_YFrameDelay 
    RTS 
...
CinematicFunction_CeresGoesBoom_SmallCeresExplosion_FadingIn:
    LDA.W $1995 : CLC : ADC.W #$1000 : STA.W $1995 
    LDA.W $1997 : ADC.W #$0000 : STA.W $1997 
...
InitFunction_CinematicSpriteObject_CeresExplosion1:
    ...
    EOR.W #$FFFF 
    INC A 
    CLC : ADC.B $80 : CLC : ADC.W .Xoffset,X : STA.W $1A7D,Y 
    LDA.W $1997 
    EOR.W #$FFFF 
    INC A 
    CLC : ADC.B $82 : CLC : ADC.W .Yoffset,X : STA.W $1A9D,Y 
    LDA.W #$0A00 : STA.W $1ABD,Y 
    RTS 
...
PreInst_CeresExplosionSpawner_SpawnExplosion2EveryCFrames:
    LDA.W $1F51 : CMP.W #CinematicFunction_CeresGoesBoom_GunshipFlyingAway : BNE .notGunshipFlyingAway 
    LDA.W #RTS_8B93D9 : STA.W $1B3D,X 
    RTS 
...
PreInstruction_CinematicSpriteObject_CeresExplosion:
    LDA.W $1AFD,X : SEC : SBC.W #$1000 : STA.W $1AFD,X 
    LDA.W $1A9D,X : SBC.W #$0000 : STA.W $1A9D,X 
    LDA.W $1ADD,X : CLC : ADC.W #$4000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0000 : STA.W $1A7D,X 
    RTS 
...
InitFunction_CinematicSpriteObject_CeresFinalExplosion:
    ...
    EOR.W #$FFFF 
    INC A 
    CLC : ADC.B $80 : STA.W $1A7D,Y 
    LDA.W $1997 
    EOR.W #$FFFF 
    INC A 
    CLC : ADC.B $82 : STA.W $1A9D,Y 
    LDA.W #$0A00 : STA.W $1ABD,Y 
    RTS 
...
CinematicFunction_CeresGoesBoom_GunshipFlyingAway:
    ...
    LDA.B #$37 : STA.B $72 
    REP #$20 
    LDA.W $1991 : CLC : ADC.W #$0000 : STA.W $1991 
    LDA.W $1993 : ADC.W #$0002 : STA.W $1993 
    LDA.W $198D 
    DEC A 
    AND.W #$00FF 
    STA.W $198D 
    LDA.W $198F : CMP.W #$0010 : BMI .zoomLessThan10 
    SEC : SBC.W #$0010 : STA.W $198F 
    RTS 
...
CinematicFunction_CeresGoesBoom_FadeOut:
    ...
    STZ.W $0723 
    STZ.W $0725 
    LDA.W $0998 : CMP.W #$0025 : BEQ .gameState25 
...
PreInstruction_CinematicSpriteObject_Zebes:
    LDA.W $1F51 : CMP.W #RTS_8BCADE : BNE .return 
...
PreInstruction_Zebes_SlideSceneAway:
    LDA.W $1B7D,X : CLC : ADC.W #$0040 : STA.W $1B7D,X 
    XBA 
    PHA 
    AND.W #$FF00 
    STA.B $14 
    ...
    AND.W #$00FF 
    STA.B $12 
    LDA.W $1AFD,X : SEC : SBC.B $14 : STA.W $1AFD,X 
    LDA.W $1A9D,X : SBC.B $12 : STA.W $1A9D,X 
...
PreInstruction_ZebesStars5_SlideSceneAway:
    LDA.W $1B7D,X : CLC : ADC.W #$0020 : STA.W $1B7D,X 
    XBA 
    PHA 
    AND.W #$FF00 
    STA.B $14 
...
CinematicFunction_FlyToZebes_FlyingToZebes_DriftingRight:
    LDA.W $1995 : CLC : ADC.W #$2000 : STA.W $1995 
    LDA.W $1997 : ADC.W #$0000 : STA.W $1997 
    LDA.W $1991 : SEC : SBC.W #$8000 : STA.W $1991 
    LDA.W $1993 : SBC.W #$0000 : STA.W $1993 
    LDA.W $198F : CMP.W #$0480 : BPL .zoomLessThan480 
...
CinematicFunction_FlyToZebes_FlyingToZebes_TurningLeft:
    ...
    LDA.W $198D : CMP.W #$00E0 : BEQ .return 
...
CinematicFunction_FlyToZebes_FlyingToZebes_DriftingLeft:
    ...
    LDA.W $198F : CMP.W #$2000 : BPL .zoomLessThan2000 
    CLC : ADC.W #$0020 : STA.W $198F 
    RTS 
...
HandleFinalScreen_CinematicBGObjects:
    LDA.W $1F51 : CMP.W #CinematicFunction_PostCredits_FadeFromWhite : BMI .return 
    JSR.W Handle_CinematicBGObjects 
...
.zoomOut:
    LDA.W $198F : CLC : ADC.W #$0002 : STA.W $198F 
...
CinematicFunction_Ending_ZebesDestruction1_CloudsOnTopBottom:
  ...
  + LDA.W $198F : CLC : ADC.W #$0003 : STA.W $198F 
    CMP.W #$0180 : BMI .return 
    JSR.W AdvanceFastScreenFadeOut 
    BCC .return 
...
CinematicFunc_Ending_SpaceView_LoadGunshipBG:
    LDA.W $1A49 : CMP.W #$0008 : BPL .timerGreaterThan7 
    ASL A 
    TAX 
    LDY.W $0334 
...
.checkColors:
    LDA.B $74 : CMP.B #$20 : BNE .setTimer 
    LDA.B $75 : CMP.B #$40 : BNE .setTimer 
    LDA.B $76 : CMP.B #$80 : BNE .setTimer 
    STZ.B $6F 
    STZ.B $72 
...
.timerExpired:
    LDA.W $198D : SEC : SBC.W #$0004 : AND.W #$00FF 
    STA.W $198D 
    LDA.W $1A4D 
    ASL #2
    TAX 
    LDA.W $1991 : CLC : ADC.W .shakingXVelocities+2,X : STA.W $1991 
    LDA.W $1993 : ADC.W .shakingXVelocities,X : STA.W $1993 
    LDA.W $1A4D 
    INC A 
    AND.W #$000F 
    STA.W $1A4D 
    LDA.W $198F : SEC : SBC.W #$0008 : STA.W $198F 
...
CinematicFunc_Ending_SpaceView_GunshipEmergence_SpinningSlow:
    JSR.W FadeOut_ZebesExplosion_AfterGlow 
    LDA.W $198D : CMP.W #$00E0 : BEQ .notE0 
    SEC : SBC.W #$0001 : AND.W #$00FF 
    STA.W $198D 
...
.notE0:
    ...
    AND.W #$0007 
    STA.W $1A4D 
    LDA.W $198F : SEC : SBC.W #$0002 : STA.W $198F 
...
.loopPalettes:
    ...
    JSL.L Decompression_HardcodedDestination 
    dl $7E8000 
    LDA.W $09E0 : CMP.W #$0003 : BPL .gameTimeOver3 
    SEP #$30 
    LDA.B #$00 : STA.W $2116 
    LDA.B #$00 : STA.W $2117 
    LDA.B #$80 : STA.W $2115 
...
CinematicFunction_PostCredits_WavySamus:
    LDA.B $BB : CLC : ADC.W #$0002 : STA.B $BB 
    DEC.W $1A49 
    BEQ .timerExpired 
    BPL .return 
...
.decTimer:
    LDA.W #$007F : STA.W $1A49 
    LDA.W $09E0 : CMP.W #$0003 : BMI .bestEnding 
...
CinematicFunc_PostCredits_IdleSamus_1_CrossFadeOutSamusSuit:
    ...
    LDY.W #$0010 
    JSR.W PaletteCrossFading_FadeOutYColorsStartingFromColorIndexX 
    LDA.W $09E0 : CMP.W #$0003 : BMI .lessThan3 
    LDX.W #$01C0 
    LDY.W #$0010 
    JSR.W PaletteCrossFading_FadeInYColorsStartingFromColorIndexX 
...
.timerExpired:
    LDA.W $09E0 : CMP.W #$0003 : BMI .gameTimeOver10 
...
CinematicFunc_PostCredits_IdleSamus2_CrossFadeOutSamusSuit:
    ...
    LDY.W #$0010 
    JSR.W PaletteCrossFading_FadeOutYColorsStartingFromColorIndexX 
    LDA.W $09E0 : CMP.W #$0003 : BMI .gameTimeUnder3 
    LDX.W #$01C0 
    LDY.W #$0010 
    JSR.W PaletteCrossFading_FadeInYColorsStartingFromColorIndexX 
...
.transitionPalette:
  ...
  + LDA.W $198D : SEC : SBC.W #$0008 : AND.W #$00FF 
    STA.W $198D 
    LDA.W $198F : SEC : SBC.W #$0040 : STA.W $198F 
...
TransferPostCreditsSuperMetroidIconToVRAM:
    ...
    LDA.W $1A4D : CMP.W #$0006 : BMI .loading 
...
.loopTanks:
    ...
    NOP #7
    REP #$20 
    LDA.W $4214 : CLC : ADC.B $12 : STA.B $12 
    DEX #2
    BPL .loopTanks 
    LDX.W #$0014 
...
.loopProcess:
    ...
    PHA 
    AND.W #$FF00 
    CMP.W #$0400 : BMI .lessThan4 
    PLA 
    AND.W #$00FF 
    ASL #3
    TAX 
    LDA.W $000C,Y : CLC : ADC.W ShootingStar_Table_Xaccel,X : CLC : ADC.W ShootingStar_Table_Xaccel,X : STA.W $000C,Y 
    LDA.W $000E,Y : CLC : ADC.W ShootingStar_Table_Yaccel,X : CLC : ADC.W ShootingStar_Table_Yaccel,X : STA.W $000E,Y 
    BRA + 
...
.lessThan4:
  ...
  + STA.B $12 
    ...
    AND.W #$FF00 
    STA.B $14 
    LDA.W $0004,Y : CLC : ADC.B $14 : STA.W $0004,Y 
    LDA.W $0002,Y : ADC.B $12 : STA.W $0002,Y 
    LDA.W $000E,Y 
    PHA 
    XBA 
    AND.W #$00FF 
  ...
  + STA.B $12 
    ...
    AND.W #$FF00 
    STA.B $14 
    LDA.W $0008,Y : CLC : ADC.B $14 : STA.W $0008,Y 
    LDA.W $0006,Y : ADC.B $12 : STA.W $0006,Y 
...
PreInstruction_CinematicSpriteObject_CrittersEscape:
    LDA.W $1AFD,X : CLC : ADC.W #$0080 : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W #$0000 : STA.W $1A9D,X 
    LDA.W $1ADD,X : CLC : ADC.W #$0000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0001 : STA.W $1A7D,X 
...
CinematicSpriteObject_InstListPointer_PlusA_Times8:
    ASL #3
    STA.B $12 
    LDA.W $1B1D,Y : CLC : ADC.B $12 : STA.W $1B1D,Y 
    RTS 
...
PreInstruction_CineSpriteObject_SuperMetroidIcon_S_TopHalf:
    LDA.W $1A9D,X : CLC : ADC.W $1B7D,X : STA.W $1A9D,X 
    LDA.W $1A7D,X : SEC : SBC.W $1B7D,X : STA.W $1A7D,X 
    CMP.W #$018B : BPL + 
  ...
  + LDA.W $1B7D,X : CLC : ADC.W #$0002 : STA.W $1B7D,X 
    RTS 


Instruction_CineSpriteObject_SuperMetroidIcon_S_BottomHalf:
    LDA.W $1A9D,X : SEC : SBC.W $1B7D,X : STA.W $1A9D,X 
    LDA.W $1A7D,X : CLC : ADC.W $1B7D,X : STA.W $1A7D,X 
    CMP.W #$017E : BMI + 
...
Instruction_CinematicSpriteObject_ZebesExplosion_Stars_Left:
    LDA.W $1F51 : CMP.W #CinematicFunc_Ending_SpaceView_GunshipEmergence_SpinningFast : BNE .return 
...
PreInst_CineSpriteObject_ZebesExplosion_Stars_Left_Moving:
    LDA.W $1AFD,X : CLC : ADC.W #$FFE0 : STA.W $1AFD,X 
    LDA.W $1B7D,X : ADC.W #$FFFF : STA.W $1B7D,X 
    LDA.W $1ADD,X : CLC : ADC.W $1AFD,X : STA.W $1ADD,X 
...
PreInstruction_CineSpriteObject_ZebesExplosion_AfterGlow:
    LDA.W $1F51 : CMP.W #RTS_8BDE63 : BNE .return 
    LDA.W #$0001 : STA.W $1B5D,X 
    LDA.W #InstList_CinematicSpriteObject_Delete_duplicate : STA.W $1B1D,X 
...
PreInstruction_CinematicSpriteObject_Text:
    LDA.W $1F51 : CMP.W #CinematicFunction_Credits_Setup : BNE .return 
    LDA.W #$0001 : STA.W $1B5D,X 
    LDA.W #InstList_CinematicSpriteObject_Delete_duplicate : STA.W $1B1D,X 
...
PreInstruction_CinematicSpriteObject_YellowClouds_Top:
    LDA.W $198F : CMP.W #$00B0 : BPL .return 
...
PreInstruction_CinematicSpriteObject_YellowClouds_Top_Moving:
    LDA.W $1AFD,X : CLC : ADC.W #$0000 : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W #$0001 : STA.W $1A9D,X 
    RTS 
...
PreInstruction_CineSpriteObject_YellowClouds_Bottom_Moving:
    LDA.W $1AFD,X : SEC : SBC.W #$0000 : STA.W $1AFD,X 
    LDA.W $1A9D,X : SBC.W #$0001 : STA.W $1A9D,X 
    RTS 


PreInstruction_CinematicSpriteObject_YellowClouds_Right:
    LDA.W $198F : CMP.W #$0060 : BMI .return 
...
PreInstruction_CineSpriteObject_YellowClouds_Right_Moving:
    LDA.W $1AFD,X : SEC : SBC.W #$0000 : STA.W $1AFD,X 
    LDA.W $1A9D,X : SBC.W #$0002 : STA.W $1A9D,X 
    LDA.W $1ADD,X : SEC : SBC.W #$0000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : SBC.W #$0001 : STA.W $1A7D,X 
    RTS 
...
PreInstruction_CineSpriteObject_YellowClouds_Left_Moving:
    LDA.W $1AFD,X : CLC : ADC.W #$0000 : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W #$0002 : STA.W $1A9D,X 
    LDA.W $1ADD,X : CLC : ADC.W #$0000 : STA.W $1ADD,X 
    LDA.W $1A7D,X : ADC.W #$0001 : STA.W $1A7D,X 
    RTS 
...
PreInstruction_CinematicSpriteObject_Samus_Idle:
    LDA.W $1F51 : CMP.W #RTS_8BDB9D : BNE .return 
    LDA.W #$0001 : STA.W $1B5D,X 
    LDA.W #InstList_CinematicSpriteObject_Delete_duplicate : STA.W $1B1D,X 
...
Inst_CineSpriteObject_SpawnSuitlessSamus_LettingHairDown:
    ...
    LDA.W $1A9D,X : CMP.W #$FFB0 : BPL .return 
    SEP #$20 
    LDA.B #$03 : STA.B $52 
    REP #$20 
    LDA.W #$0001 : STA.W $1B5D,X 
...
Instruction_CinematicSpriteObject_SpawnSuitedSamusJump:
    PHY 
    LDA.W $09E0 : CMP.W #$000A : BMI .gameTimeUnder10 
...
PreInstruction_CinematicSpriteObject_Samus_Jump_Falling:
    ...
    JSR.W MoveJumpingEndingSamus 
    LDA.W $1A9D,X : CMP.W #$0088 : BMI .return 
...
MoveJumpingEndingSamus:
    LDA.W $0B2C : CLC : ADC.W #$3800 : STA.W $0B2C 
    LDA.W $0B2E : ADC.W #$0000 : STA.W $0B2E 
    LDA.W $1AFD,X : CLC : ADC.W $0B2C : STA.W $1AFD,X 
    LDA.W $1A9D,X : ADC.W $0B2E : STA.W $1A9D,X 
    RTS 


TransferPostCreditsSamusBeamToVRAM:
    PHX : PHY 
    LDA.W $1A4D : CMP.W #$0010 : BPL .return 
    ASL A 
    TAY 
    LDX.W $0330 
    LDA.W #$0800 : STA.B $D0,X 
